...
export class PurchaseOrderOverseaController {
  ...
  async purchaseOrderOverseaList(@Query() body: listOverseaDto) {
    ...
    const purchaseBy = body.PurchaseBy
      ? StringValidator(body.PurchaseBy)
      : undefined;
    const resquestBy = body.RequestBy
      ? StringValidator(body.RequestBy)
      : undefined;
    const pageNum = body.page ? NumberValidator(+body.page) : undefined;
    const startDateValid = body.startDate
      ? DateValidator(body.startDate)
      : undefined;
