...
export class PurchaseOrderOverseaService {
  ...
  async purchaseOrderOverseaByType(poType: string) {
    try {
      let typequery = '';
      if (poType === 'Asset') {
      ...
      const query = `
      select 'Pine-Pacific' as Companyname, s.DateArrive as ReciveDate, po.DateOrder as PoDate, po.PurchaseID as PoID, pod.SProductName as ProductName, sup.SupplierName as SupplierName, pr.Purpose as Purpose,
pod.Amount as Amount, pr.Division as Dep, po.PurchaseOfficer as PurchaseBy,
CASE 
         WHEN pod.ProductID LIKE '5%' THEN 'Asset'
         ELSE 'Non-Asset'
